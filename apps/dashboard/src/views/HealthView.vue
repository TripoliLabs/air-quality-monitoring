...
const maintenanceCount = computed(
  () => store.allSensors.filter((s) => s.definition.status === 'maintenance').length,
);

const avgBattery = computed(() => {
  const withReadings = store.allSensors.filter((s) => s.latest);
  if (withReadings.length === 0) return 0;
  const sum = withReadings.reduce((acc, s) => acc + batteryPercent(s.latest!.batteryMv), 0);
...
const avgSignal = computed(() => {
  const withReadings = store.allSensors.filter((s) => s.latest);
  if (withReadings.length === 0) return 0;
  const sum = withReadings.reduce((acc, s) => acc + s.latest!.signalStrength, 0);
  return Math.round(sum / withReadings.length);
});

// Alerts
const lowBatteryCount = computed(
  () => store.allSensors.filter((s) => s.latest && batteryPercent(s.latest.batteryMv) < 30).length,
);
const weakSignalCount = computed(
  () => store.allSensors.filter((s) => s.latest && s.latest.signalStrength < -70).length,
);
  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <div class="rounded-xl border border-gray-800 bg-gray-900 p-4">
        ...
        <div v-else class="flex h-full items-center justify-center py-10 text-sm text-green-400">
        ...
        <Column :header="t('health.battery')" sortable sort-field="latest.batteryMv">
          <template #body="{ data }: { data: SensorTimeSeries }">
            <span
              ...
              class="text-gray-300"
              :class="{
                'text-red-400': batteryPercent(data.latest.batteryMv) < 30,
                'text-yellow-400':
          ...
          <template #body="{ data }: { data: SensorTimeSeries }">
            <span
              ...
              :class="{
                ...
                'text-yellow-400':
                  data.latest.signalStrength >= -70 && data.latest.signalStrength <= -50,
              }"
            >
