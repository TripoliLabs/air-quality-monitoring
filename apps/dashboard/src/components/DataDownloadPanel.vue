...
const parsedTo = computed(() =>
  toDate.value ? new Date(new Date(toDate.value).getTime() + 86400000 - 1) : null,
);
...
function doDownload(): void {
  ...
  if (format.value === 'csv') {
    const csv = generateCSV(filteredSensors.value, parsedFrom.value, parsedTo.value, locale.value);
...
<template>
  <div class="rounded-xl border border-gray-800 bg-gray-900 p-4">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
      <!-- Date Range -->
      <div>
        <label class="mb-1 block text-xs text-gray-400">{{ t('download.dateRange') }}</label>
      ...
      <!-- Sensor Selection -->
      <div>
        <label class="mb-1 block text-xs text-gray-400">{{ t('download.sensors') }}</label>
      ...
      <div class="flex flex-col justify-between">
        <div>
          <label class="mb-1 block text-xs text-gray-400">{{ t('download.format') }}</label>
          <div class="flex gap-3">
            <label class="flex cursor-pointer items-center gap-1.5 text-sm text-gray-200">
              <input v-model="format" type="radio" value="csv" class="accent-emerald-500" />
              CSV
            </label>
            <label class="flex cursor-pointer items-center gap-1.5 text-sm text-gray-200">
              <input v-model="format" type="radio" value="json" class="accent-emerald-500" />
              JSON
            </label>
          </div>
        </div>
