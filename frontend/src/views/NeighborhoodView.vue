...
const averages = computed(() => {
  ...
  const pm10 =
    withReadings.reduce((sum, s) => sum + (s.latest?.pm10 ?? 0), 0) / count;
  const temp =
    withReadings.reduce((sum, s) => sum + (s.latest?.temperature ?? 0), 0) /
    count;
  const humidity =
    withReadings.reduce((sum, s) => sum + (s.latest?.humidity ?? 0), 0) / count;
  const aqi =
    Math.round(
      withReadings.reduce((sum, s) => sum + (s.latest?.aqi ?? 0), 0) / count,
    );
  return {
    aqi,
    ...
    <div class="flex items-center gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-sm text-gray-400">
          {{ sensors.length }} {{ t('neighborhood.sensors') }}
        </p>
      </div>
